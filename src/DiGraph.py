...
class DiGraph(GraphInterface):
    ...
    def remove_node(self, node_id: int) -> bool:
        ...
        for key in src_map.keys():
            ...
            removed_edge = curr_father.remove_dest(curr_node.get_key())  # remove from destMap
            self._MCount += 1
            self._parsed_edges.remove(removed_edge)
        dest_map = curr_node.get_destMap()
        for key in dest_map.keys():
            curr_child = dest_map.get(key)
            removed_edge = curr_child.remove_src(curr_node.get_key())  # remove from srcMap
            self._MCount += 1
            self._parsed_edges.remove(removed_edge)
        return True
    ...
    def remove_edge(self, node_id1: int, node_id2: int) -> bool:
        ...
        if src_node is None or dest_node is None:
            return False
        removed_edge = src_node.remove_dest(node_id2)  # remove from destMap
        if removed_edge is None:
            return False
        dest_node.remove_src(node_id1)  # remove from srcMap
        self._MCount += 1
        self._parsed_edges.remove(removed_edge)
        return True
